...
import { cardFilename, downloadBlob, renderInstagramCover, renderMatchCardPng } from "../lib/cardPng";
...
export function MatchAwardCard({ card, game }: { card: MatchCard; game?: Pelada }) {
  return <article className={`match-card match-card-${card.categoria}`}>
    ...
    {(card.snapshot_vitorias ?? 0) > 0 && <em className="match-card-wins">{card.snapshot_vitorias} {card.snapshot_vitorias === 1 ? "vitória" : "vitórias"}</em>}
...
export function MatchCardsManager() {
  const [gameId, setGameId] = useState(""),
    ...
    [toast, setToast] = useState(""), [downloading, setDownloading] = useState(false);
  ...
  return <section className="cards-manager"><header><span><p className="eyebrow">CARDS DA PELADA</p><h2>Gerar e publicar</h2></span>
  ...
  </header>{cards.length > 0 && <nav className="cards-downloads"><button className="secondary" disabled={downloading} onClick={() => void (async () => { setDownloading(true); try { downloadBlob(await renderInstagramCover(game), "capa-destaques.png"); } catch (error) { setToast(error instanceof Error ? error.message : "Não foi possível gerar a capa."); } finally { setDownloading(false); } })()}>BAIXAR CAPA PNG</button></nav>}{categories.map(([category, title]) => {
    const voteResults = awards.filter(item => item.categoria === category).sort((a, b) => b.votos - a.votos), teamWins = Math.max(0, ...teams.map(item => item.vitorias ?? 0)),
      ...
      candidateOptions = category === "goleiro_destaque"
        ? (winners.length ? winners : keepersInGame.map(item => ({ jogador_id: item.jogador_id, nome: name(item) })))
        : (category === "artilheiro" || category === "time_destaque") && winners.length ? winners
          : category === "time_destaque" ? [] : participantsInGame.map(item => ({ jogador_id: item.jogador_id, nome: name(item) })),
    ...
    return <article className="card-manager-row" key={category}><div><h3>{title}</h3>
      {!winners.length ? <small>{category === "artilheiro" ? "Sem gols registrados nesta pelada." : category === "time_destaque" ? "Registre as vitórias dos times primeiro." : category === "goleiro_destaque" ? "Sem votos para melhor goleiro. Escolha um goleiro." : "Sem votos. Escolha um participante."}</small>
      ...
      <select value={choice} onChange={event => setChoices(old => ({ ...old, [category]: event.target.value }))}><option value="">Escolha…</option>
        {candidateOptions.map(item => <option value={item.jogador_id} key={item.jogador_id}>{item.nome}</option>)}
...
export function MatchCardsGallery({ game }: { game?: Pelada }) {
  const [open, setOpen] = useState<MatchCard | null>(null), [downloading, setDownloading] = useState(false), state = useLoad(() => game ? matchCards(game.id) : Promise.resolve([]), game?.id);
