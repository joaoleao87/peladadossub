...
import {
  deleteMatchCardImage, generateMatchCard, matchAwards, matchCardImageUrl,
...
const categories: [CardCategory, string][] = [
  ...
  ["negativo", "Quem quebrou mais"], ["artilheiro", "Artilheiro"],
];
...
export function MatchAwardCard({ card, game }: { card: MatchCard; game?: Pelada }) {
  return <article className={`match-card match-card-${card.categoria}`}>
    {card.snapshot_foto_url ? <img src={card.snapshot_foto_url} alt="" /> : <div className="match-card-avatar">{card.snapshot_nome[0]}</div>}
    <small>PELADA DOS SUB</small><h2>{card.titulo}</h2><strong>{card.snapshot_nome}</strong>
    <span>{card.snapshot_time || "Companheiros não informados"} • {card.snapshot_gols} {card.snapshot_gols === 1 ? "gol" : "gols"}</span>
...
export function MatchCardsManager() {
  ...
  const state = useLoad(async () => {
    ...
    if (!id) return { games, list: [], awards: [], cards: [] };
    const [list, awards, cards] = await Promise.all([participants(id), matchAwards(id), matchCards(id)]);
    return { games, list, awards, cards };
  }, gameId);
  if (state.loading) return <Spinner />;
  if (state.error) return <ErrorState message={state.error} retry={state.reload} />;
  const { games, list, awards, cards } = state.data!, id = gameId || games[0]?.id || "", game = games.find(item => item.id === id),
  ...
  return <section className="cards-manager"><header><span><p className="eyebrow">CARDS DA PELADA</p><h2>Gerar e publicar</h2></span>
    ...
    const voteResults = awards.filter(item => item.categoria === category).sort((a, b) => b.votos - a.votos),
      top = category === "artilheiro" ? Math.max(0, ...participantsInGame.map(item => item.gols ?? 0)) : voteResults[0]?.votos ?? 0,
      winners = category === "artilheiro"
      ...
      unit = category === "artilheiro" ? (top === 1 ? "gol" : "gols") : (top === 1 ? "voto" : "votos");
    return <article className="card-manager-row" key={category}><div><h3>{title}</h3>
      {!winners.length ? <small>{category === "artilheiro" ? "Sem gols registrados nesta pelada." : "Sem votos. Escolha um participante."}</small>
      ...
      <select value={choice} onChange={event => setChoices(old => ({ ...old, [category]: event.target.value }))}><option value="">Escolha…</option>
        {(category === "artilheiro" && winners.length ? winners : participantsInGame.map(item => ({ jogador_id: item.jogador_id, nome: name(item) }))).map(item => <option value={item.jogador_id} key={item.jogador_id}>{item.nome}</option>)}
