...
import { cardFilename, downloadBlob, downloadMatchCardsZip, renderInstagramCover, renderMatchCardPng } from "../lib/cardPng";
...
export function MatchCardsManager() {
  ...
  </header>{cards.length > 0 && <nav className="cards-downloads"><button className="secondary" disabled={downloading} onClick={() => void (async () => { setDownloading(true); try { downloadBlob(await renderInstagramCover(cards, game), "capa-destaques.png"); } catch (error) { setToast(error instanceof Error ? error.message : "Não foi possível gerar a capa."); } finally { setDownloading(false); } })()}>BAIXAR CAPA PNG</button><button disabled={downloading} onClick={() => void (async () => { setDownloading(true); try { await downloadMatchCardsZip(cards, game); } catch (error) { setToast(error instanceof Error ? error.message : "Não foi possível gerar o conjunto."); } finally { setDownloading(false); } })()}>{downloading ? "GERANDO…" : "BAIXAR CONJUNTO"}</button></nav>}{categories.map(([category, title]) => {
