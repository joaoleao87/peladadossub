...
import { CARD_POSITIONS, PLAYER_CARD_STATUS_LABEL, playerCardStatus } from "../lib/playerCard";
...
export function PlayerCardsAdmin() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const card: PlayerCardData = { ...draft, display_name: String(form.get("display_name") || "") || null, position: String(form.get("position") || "") as PlayerCardPosition || null, overall: number(form.get("overall")), pace: number(form.get("pace")), shooting: number(form.get("shooting")), passing: number(form.get("passing")), dribbling: number(form.get("dribbling")), defending: number(form.get("defending")), physical: number(form.get("physical")) };
    setBusy(true);
  ...
  if (selected && draft) return <section className="player-card-editor">
    ...
    <div className="card-editor-layout"><PlayerCard player={draft} cardType={draft.card_type} mode="admin" />
      <form className="card-editor-form" onSubmit={submit} onChange={event => {
        ...
        setDraft(old => old ? { ...old, [key]: ["overall","pace","shooting","passing","dribbling","defending","physical"].includes(key) ? number(value) : value } : old);
      }}>
        ...
        <label>Overall<input name="overall" type="number" min="1" max="99" step="1" defaultValue={draft.overall ?? ""} /></label>
        {[["pace","Ritmo (RIT)"],["shooting","Finalização (FIN)"],["passing","Passe (PAS)"],["dribbling","Drible (DRI)"],["defending","Defesa (DEF)"],["physical","Físico (FÍS)"]] .map(([key,label]) => <label key={key}>{label}<input name={key} type="number" min="1" max="99" step="1" defaultValue={draft[key as keyof PlayerCardData] as number ?? ""} /></label>)}
  ...
  return <section className="player-cards-admin"><p className="eyebrow">CARTINHAS DOS JOGADORES</p><h2>Cartinhas dos jogadores</h2><p>Configure overall, posição, tipo de carta e atributos dos jogadores.</p>
