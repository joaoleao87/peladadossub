...
export function Layout(){const {profile}=useAuth();return <><header><img className="header-logo" src="/logo.png" alt="Pelada dos Sub"/><CalendarDays width={22}/></header><main><Outlet/></main><nav className="bottom-nav">{links.map(({to,label,icon:Icon})=><NavLink key={to} to={to} end={to==='/'}><Icon/><span>{label}</span></NavLink>)}{(profile?.role==='admin'||profile?.role==='superadmin')&&<NavLink to="/admin"><Shield/><span>Admin</span></NavLink>}{profile?.role==='superadmin'&&<NavLink to="/superadmin"><Users/><span>Users</span></NavLink>}</nav></>}
