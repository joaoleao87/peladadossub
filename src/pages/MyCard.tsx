...
export function MyCard() {
  ...
  const currentPlayer = player,
    displayCard = card ? { ...card, resolved_photo_url: card.resolved_photo_url || profile?.foto_url || null } : null,
    editableCard: PlayerCardData = displayCard || { player_id: currentPlayer.id, display_name: null, position: null, overall: null, pace: null, shooting: null, passing: null, dribbling: null, defending: null, physical: null, card_type: "normal", photo_url: null, photo_scale: 1, photo_position_x: 0, photo_position_y: 0, resolved_photo_url: profile?.foto_url || null };
  ...
  return <section className="my-card-page">
    ...
    {ready && displayCard ? <PlayerCard player={displayCard} /> : <div className="card-empty">
      {(displayCard?.resolved_photo_url || profile?.foto_url) && <img className="card-empty-photo" src={displayCard?.resolved_photo_url || profile?.foto_url || ""} alt="Foto do jogador" />}
