...
function MatchController({peladaId,onBack}:{peladaId:string;onBack:()=>void}) {
  ...
  function tossCoin(){setCoinToss(null);setCoinRoll(value=>value+1);setTossing(true);const result=Math.random()<.5?"cara":"coroa";setTimeout(()=>{setCoinToss(result);setTossing(false)},1200)}
  ...
  return <section className="match-control">
    ...
    {tieBreak&&match.status==="CREATED"&&<section className="match-finish-confirm panel"><b>DESEMPATE PENDENTE</b><p>Time {tieBreak.team_home} e Time {tieBreak.team_away} empataram. Escolha quem entra na frente.</p><div className={`coin ${tossing?"spinning":""}`} key={coinRoll}><img src={coinToss==="coroa"?coinTails:coinHeads} alt={coinToss??"Moeda"}/></div>{coinToss!==null&&<strong className="coin-result">RESULTADO: {coinToss.toUpperCase()}</strong>}<button className="secondary" disabled={busy||tossing} onClick={tossCoin}>CARA OU COROA</button><div><button disabled={busy||tossing} onClick={()=>resolveTieBreak(tieBreak.team_home)}>TIME {tieBreak.team_home} ENTRA</button><button disabled={busy||tossing} onClick={()=>resolveTieBreak(tieBreak.team_away)}>TIME {tieBreak.team_away} ENTRA</button></div></section>}
