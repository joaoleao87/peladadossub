...
function MatchController({peladaId,onBack}:{peladaId:string;onBack:()=>void}) {
  ...
  const tieBreak=snapshot.pendingTieBreaks[0];
  function finish(){void run("finalizar_partida_controlada",{p_match_id:matchId,p_client_event_id:crypto.randomUUID(),p_occurred_at:correctedNow(),p_match_clock_ms:0},"Partida finalizada. Próximos times chamados.").then(done=>{if(done)setTwoGoalTeam(null)})}
  ...
  function tossCoin(){setCoinToss(null);setCoinRoll(value=>value+1);setTossing(true);const result=Math.random()<.5?"cara":"coroa",flip=window.setInterval(()=>setCoinFace(face=>face==="cara"?"coroa":"cara"),120);setTimeout(()=>{clearInterval(flip);setCoinFace(result);setCoinToss(result);setTossing(false)},1200)}
  function resolveTieBreak(team:number){void run("resolver_desempate_entrada",{p_match_id:matchId,p_team_id:team},`Time ${team} entra na frente.`,()=>undefined).then(done=>{if(done){setCoinToss(null);setTossing(false)}})}
  ...
  return <section className="match-control">
    ...
    {tieBreak&&match.status==="CREATED"&&<section className="match-finish-confirm panel"><b>DESEMPATE PENDENTE</b><p>Time {tieBreak.team_home} e Time {tieBreak.team_away} empataram. Escolha quem entra na frente.</p><div className={`coin ${tossing?"spinning":""}`} key={coinRoll}><img src={coinFace==="coroa"?coinTails:coinHeads} alt={coinToss??"Moeda"}/></div>{coinToss!==null&&<strong className="coin-result">RESULTADO: {coinToss.toUpperCase()}</strong>}<button className="secondary" disabled={busy||tossing} onClick={tossCoin}>CARA OU COROA</button><div><button disabled={busy||tossing} onClick={()=>resolveTieBreak(tieBreak.team_home)}>TIME {tieBreak.team_home} ENTRA</button><button disabled={busy||tossing} onClick={()=>resolveTieBreak(tieBreak.team_away)}>TIME {tieBreak.team_away} ENTRA</button></div></section>}
    ...
    <MatchEvents snapshot={snapshot} onSaved={saveGoal}/>
