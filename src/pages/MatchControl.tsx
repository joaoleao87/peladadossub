...
function MatchController({peladaId,onBack}:{peladaId:string;onBack:()=>void}) {
  ...
  function tossCoin(){if(!tieBreak)return;setCoinToss(null);setTossing(true);const result=Math.random()<.5?tieBreak.team_home:tieBreak.team_away;setTimeout(()=>{setCoinToss(result);setTossing(false)},1200)}
  ...
  return <section className="match-control">
    ...
    {tieBreak&&match.status==="CREATED"&&<section className="match-finish-confirm panel"><b>DESEMPATE PENDENTE</b><p>Time {tieBreak.team_home} e Time {tieBreak.team_away} empataram. Escolha quem entra na frente.</p><div className={`coin ${tossing?"spinning":""}`}>{tossing?"🪙":coinToss===tieBreak.team_home?"CARA":coinToss===tieBreak.team_away?"COROA":"🪙"}</div>{coinToss!==null&&<strong className="coin-result">Deu {coinToss===tieBreak.team_home?"cara":"coroa"}: Time {coinToss}</strong>}<button className="secondary" disabled={busy||tossing} onClick={tossCoin}>CARA OU COROA</button><div><button disabled={busy||tossing} onClick={()=>resolveTieBreak(tieBreak.team_home)}>TIME {tieBreak.team_home} ENTRA</button><button disabled={busy||tossing} onClick={()=>resolveTieBreak(tieBreak.team_away)}>TIME {tieBreak.team_away} ENTRA</button></div></section>}
