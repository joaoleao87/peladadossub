...
function CoinToss({onTossingChange}:{onTossingChange?:(value:boolean)=>void}) {
  ...
  function toss(){setResult(null);setRoll(value=>value+1);setTossing(true);onTossingChange?.(true);const next=Math.random()<.5?"cara":"coroa",flip=window.setInterval(()=>setFace(value=>value==="cara"?"coroa":"cara"),120);setTimeout(()=>{clearInterval(flip);setFace(next);setResult(next);setTossing(false);onTossingChange?.(false)},1200)}
...
function MatchController({peladaId,onBack,showEvents=true}:{peladaId:string;onBack?:()=>void;showEvents?:boolean}) {
  ...
  function resolveTieBreak(team:number){void run("resolver_desempate_entrada",{p_match_id:matchId,p_team_id:team},`Time ${team} entra na frente.`,()=>undefined).then(done=>{if(done)setTossing(false)})}
...
export function CoinTossPage() { return <section className="match-control"><header><Link className="link" to="/">← VOLTAR</Link></header><section className="match-finish-confirm panel"><b>CARA OU COROA</b><p>Faça o sorteio antes de escolher quem entra.</p><CoinToss/></section></section> }
