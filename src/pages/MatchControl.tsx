...
function MatchController({peladaId,onBack}:{peladaId:string;onBack:()=>void}) {
  ...
  const alarmed=useRef(""),offsetRef=useRef(0);
  ...
  return <section className="match-control">
    ...
    <footer><span>{navigator.onLine?"● ONLINE":"○ OFFLINE"}</span>{pending>0&&<b>{pending} pendente{pending===1?"":"s"}</b>}<small>Na tela bloqueada: anterior = gol esquerdo, próximo = gol direito, avançar = lance e voltar = desfazer.</small></footer>
    <Toast message={toast}/>
  </section>
}
