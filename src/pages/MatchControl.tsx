...
function MatchController({peladaId,onBack}:{peladaId:string;onBack:()=>void}) {
  ...
  function finish(winner?:number){
    if(!match)return;
    if(match.score_home===match.score_away&&!winner){setTwoGoalTeam(null);setTieBreak(true);return}
    void run("finalizar_partida_controlada",{p_match_id:matchId,p_client_event_id:crypto.randomUUID(),p_occurred_at:correctedNow(),p_match_clock_ms:0,p_winner_team:winner??null},"Partida finalizada. Próximos times chamados.").then(done=>{if(done){setTwoGoalTeam(null);setTieBreak(false);setCoinToss(null)}})
  }
  ...
  return <section className="match-control">
    ...
    {tieBreak&&match.status==="RUNNING"&&<section className="match-finish-confirm panel"><b>PLACAR EMPATADO</b><p>Escolha quem venceu ou faça cara ou coroa antes de chamar os próximos times.</p>{coinToss===null?<button className="secondary" disabled={busy} onClick={()=>setCoinToss(Math.random()<.5?match.team_home:match.team_away)}>🪙 CARA OU COROA</button>:<p><strong>Deu {coinToss===match.team_home?"cara":"coroa"}: Time {coinToss}.</strong> Confirme ou escolha o outro time.</p>}<div><button disabled={busy} onClick={()=>finish(match.team_home)}>TIME {match.team_home} VENCEU</button><button disabled={busy} onClick={()=>finish(match.team_away)}>TIME {match.team_away} VENCEU</button><button type="button" className="link" disabled={busy} onClick={()=>{setTieBreak(false);setCoinToss(null)}}>CANCELAR</button></div></section>}
    ...
    {match.status==="CREATED"?<button className="match-start" disabled={busy} onClick={()=>{const id=crypto.randomUUID(),occurred=correctedNow();void run("iniciar_partida_controlada",{p_match_id:match.id,p_client_event_id:id,p_occurred_at:occurred},"Partida iniciada.",()=>{match.status="RUNNING";match.started_at=occurred;setTick(value=>value+1)})}}>▶ INICIAR PARTIDA</button>:<div className="match-actions">
      ...
      <button className="danger" disabled={busy} onClick={()=>{if(match.score_home===match.score_away){finish();return}if(confirm("Finalizar esta partida e chamar os próximos times?"))finish()}}>■ FINALIZAR PARTIDA</button>
    </div>}
