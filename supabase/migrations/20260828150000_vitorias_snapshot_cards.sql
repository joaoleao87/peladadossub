alter table public.pelada_cards
  add column if not exists snapshot_vitorias smallint not null default 0
  check(snapshot_vitorias between 0 and 99);

update public.pelada_cards c set snapshot_vitorias=coalesce((
  select max(pt.vitorias) from public.pelada_times pt
  where pt.pelada_id=c.pelada_id and pt.jogador_id=c.jogador_id
),0);
