alter table public.pelada_times
  add column if not exists vencedor boolean not null default false;

create or replace function public.admin_definir_time_vencedor(
  p_pelada_id uuid,
  p_time integer,
  p_vencedor boolean
...
begin
  ...
  if not exists(select 1 from public.pelada_times where pelada_id=p_pelada_id and time=p_time) then
    raise exception 'Time não encontrado';
  end if;
  if p_vencedor then
    update public.pelada_times set vencedor=false where pelada_id=p_pelada_id;
  end if;
  update public.pelada_times set vencedor=p_vencedor
  where pelada_id=p_pelada_id and time=p_time;
end $$;
revoke all on function public.admin_definir_time_vencedor(uuid,integer,boolean) from public;
grant execute on function public.admin_definir_time_vencedor(uuid,integer,boolean) to authenticated;
...
begin
  ...
  if v_time is not null then
    ...
    into v_companheiros
    from public.pelada_times pt
    join public.jogadores j on j.id=pt.jogador_id
    left join public.profiles pr on pr.id=j.user_id
  ...
  on conflict(pelada_id,categoria) do update set jogador_id=excluded.jogador_id,titulo=excluded.titulo,
    ...
    snapshot_gols=excluded.snapshot_gols,imagem_path=null,liberado=false,liberado_em=null,gerado_por=auth.uid(),
    gerado_em=now(),updated_at=now();
end $$;
