...
alter table public.pelada_cards add constraint pelada_cards_categoria_check
  check(categoria in('destaque','surpresa','negativo','artilheiro','time_destaque','goleiro_destaque'));
...
begin
  ...
  if p_categoria not in('destaque','surpresa','negativo','artilheiro','time_destaque','goleiro_destaque') then raise exception 'Categoria inválida'; end if;
  ...
  if p_categoria='goleiro_destaque' and not exists(select 1 from public.pelada_participantes where pelada_id=p_pelada_id and jogador_id=p_jogador_id and categoria='goleiro') then
  ...
  if p_categoria='time_destaque' then
    ...
    v_titulo='Time Destaque';v_foto=null;v_companheiros='Time '||v_time;v_gols=0;
  else
    ...
    v_titulo=case p_categoria when 'destaque' then 'Destaque' when 'surpresa' then 'Surpresa' when 'artilheiro' then 'Artilheiro' when 'goleiro_destaque' then 'Goleiro Destaque' else 'Quem quebrou mais' end;
  end if;
  insert into public.pelada_cards(pelada_id,categoria,jogador_id,titulo,snapshot_nome,snapshot_foto_url,snapshot_time,snapshot_gols,snapshot_vitorias,snapshot_membros,gerado_por)
  values(p_pelada_id,p_categoria,p_jogador_id,v_titulo,v_nome,v_foto,v_companheiros,coalesce(v_gols,0),coalesce(v_vitorias,0),v_membros,auth.uid())
  on conflict(pelada_id,categoria) do update set jogador_id=excluded.jogador_id,titulo=excluded.titulo,snapshot_nome=excluded.snapshot_nome,
    snapshot_foto_url=excluded.snapshot_foto_url,snapshot_time=excluded.snapshot_time,snapshot_gols=excluded.snapshot_gols,snapshot_vitorias=excluded.snapshot_vitorias,
